...
third_parties = ('futures-', 'six-', 'datastax_db_advanced_auth_client_plugin-')
...
DSE_CQL_HTML_FALLBACK = 'https://docs.datastax.com/en/cql/dse-6.9/index.html'
...
class DSEShell(OriginalShell):
    def __init__(self, hostname, port, color=False,
                 ...
                 is_subshell=False,
                 debug=False):
        from dselib.authfactory import get_auth_provider, VALID_AUTH_SECTIONS
        if username:
            if not password:
                password = getpass.getpass()

        auth = cqlsh.options.auth if cqlsh.options and hasattr(cqlsh.options, "auth") else None
        if auth is not None and auth not in VALID_AUTH_SECTIONS:
            raise Exception('Argument auth must be one of {0}'.format(VALID_AUTH_SECTIONS))

        if cqlsh.options is not None and hasattr(cqlsh.options, "debug") and cqlsh.options.debug is not None:
            debug = cqlsh.options.debug

        auth_provider = get_auth_provider(cqlsh.CONFIG_FILE, os.environ, hostname, username, password, auth, debug) \
    ...
    def do_help(self, parsed):
        ...
        Full documentation available at:
          https://docs.datastax.com/en/cql/dse-6.9/index.html
        """
        ...
        if not topics:
            ...
            print("  https://docs.datastax.com/en/cql/dse-6.9/reference/cqlsh-commands/cqlsh-commands-toc.html")
            print('')
            ...
            print("  https://docs.datastax.com/en/cql/dse-6.9/reference/cql-commands/commands-toc.html")
            print('')
            return
        for t in topics:
