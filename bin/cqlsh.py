...
parser.add_option('--auth', help='Use authentication section defined in cqlshrc.')
    ...
    def perform_statement(self, statement):
        ...
        if m:
            ...
            for i, col in enumerate(columns):
                if col == 'field0' and values[i].startswith("'") and values[i].endswith("'"):
                    original = values[i][1:-1]  # strip quotes
                    # Prepend 0x00 byte
                    modified = '\x00' + original
                    values[i] = f"'{modified}'"
                    print(f"[DEBUG] Modified {col}: {original} -> {values[i]}")
...
def read_options(cmdlineargs, environment):
    ...
    optvalues.auth = option_with_default(configs.get, 'authentication', 'auth', None)
...
def main(options, hostname, port):
    ...
    if options.debug:
        ...
        sys.stderr.write("Using Python: %s\n" % (sys.version.splitlines()[0].strip(),))
