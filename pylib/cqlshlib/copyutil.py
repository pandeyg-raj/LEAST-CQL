...
from cassandra.cluster import Cluster, DefaultConnection, ExecutionProfile, EXEC_PROFILE_DEFAULT
from cassandra.connection import SniEndPoint
from cassandra.cqltypes import ReversedType, UserType, VectorType
...
IS_MAC_M1 = platform.system() == 'Darwin' and platform.processor() == 'arm'
...
class CopyTask(object):
    ...
    def make_params(self):
        ...
        On Windows and Mac this dictionary must be pickle-able, therefore we do not pass the
        ...
        the parent connection anyway, see CASSANDRA-11749 and CASSANDRA-16053 for more details.
        """
        shell = self.shell
        ...
        return dict(ks=self.ks,
                    ...
                    parent_cluster=None if IS_WINDOWS or IS_MAC_M1 else shell.conn,
    ...
    def _get_dest(self, source_name):
        ...
        if self.fname is None:
            ...
            try:
                ...
                printerrmsg(self.shell, "Can't open %r for writing: %s" % (source_name, e))
                return None
...
class FeedingProcess(mp.Process):
    ...
    def inner_run(self):
        ...
        try:
            ...
            printdebugmsg("PID {} - Failed to create reader: {}".format(self.pid, exc))
...
class ExportProcess(ChildProcess):
    ...
    def connect(self, host):
        ...
        execution_profiles = {EXEC_PROFILE_DEFAULT:
                              ExecutionProfile(retry_policy=ExpBackoffRetryPolicy(self),
...
class ImportConversion(object):
    ...
    def __init__(self, parent, table_meta, statement=None):
        ...
        self.converters = [self._get_converter(c.type, c.name) for c in statement.column_metadata]
    ...
    def _get_converter(self, cql_type, column_name):
        """
        ...
        def convert_vector(val, ct=cql_type, cn=column_name):
            string_coordinates = split(val)
            if len(string_coordinates) != ct.vector_size:
                raise ParseError("The length of given vector value '%d' is not equal to the vector size from the type definition '%d' of column '%s'" % (len(string_coordinates), ct.vector_size, cn))
