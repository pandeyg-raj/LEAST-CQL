...
SYSTEM_KEYSPACES = (
    ...
    'solr_admin', 'dse_system_local', 'system_virtual_schema', 'system_views', 'system_backups'
)
NONALTERBALE_KEYSPACES = (
    'system', 'system_schema', 'dse_insights_local', 'dse_system_local',
...
class Cql3ParsingRuleSet(CqlParsingRuleSet):
    ...
    columnfamily_layout_map_options = (
        ...
        ('nodesync', None,
            ('enabled', 'deadline_target_sec', 'incremental')),
    )
...
def prop_equals_completer(ctxt, cass):
    if not working_on_keyspace(ctxt):
        # we know if the thing in the property name position is "compact",
        # "clustering", "edge" or "vertex" that there won't actually be an equals sign,
        # because there are no properties by those names. there are, on the other hand,
        # table properties that start with those keywords which don't have
        # equals signs at all.
        curprop = ctxt.get_binding('propname')[-1].upper()
        if curprop in ('COMPACT', 'CLUSTERING', 'EDGE', 'VERTEX'):
            return ()
    return ['=']
...
def ks_prop_name_completer(ctxt, cass):
    ...
    if 'replication' not in optsseen:
        return ['replication']
    return ["durable_writes", "graph_engine"]
...
def backup_store_prop_name_completer(ctxt, cass):
    return ["settings"]


def backup_configuration_prop_name_completer(ctxt, cass):
    return ["frequency", "enabled"]


prop_name_completers = {"KEYSPACE": ks_prop_name_completer,
                        "SCHEMA": ks_prop_name_completer,
                        "COLUMNFAMILY": cf_prop_name_completer,
                        "TABLE": cf_prop_name_completer,
                        "STORE": backup_store_prop_name_completer,
                        "CONFIGURATION": backup_configuration_prop_name_completer}


prop_val_completers = {"KEYSPACE": ks_prop_val_completer,
                       "SCHEMA": ks_prop_val_completer,
                       "COLUMNFAMILY": cf_prop_val_completer,
                       "TABLE": cf_prop_val_completer}


prop_mapkey_completers = {"KEYSPACE": ks_prop_val_mapkey_completer,
                          "SCHEMA": ks_prop_val_mapkey_completer,
                          "COLUMNFAMILY": cf_prop_val_mapkey_completer,
                          "TABLE": cf_prop_val_mapkey_completer}


prop_mapval_completers = {"KEYSPACE": ks_prop_val_mapval_completer,
                          "SCHEMA": ks_prop_val_mapval_completer,
                          "COLUMNFAMILY": cf_prop_val_mapval_completer,
                          "TABLE": cf_prop_val_mapval_completer}


prop_mapender_completers = {"KEYSPACE": ks_prop_val_mapender_completer,
                            "SCHEMA": ks_prop_val_mapender_completer,
                            "COLUMNFAMILY": cf_prop_val_mapender_completer,
                            "TABLE": cf_prop_val_mapender_completer}


@completer_for('property', 'propname')
def prop_name_completer(ctxt, cass):
    completer_target = get_completer_target(ctxt)
    return prop_name_completers.get(completer_target)(ctxt, cass)


@completer_for('propertyValue', 'propsimpleval')
def prop_val_completer(ctxt, cass):
    completer_target = get_completer_target(ctxt)
    return prop_val_completers.get(completer_target)(ctxt, cass)


@completer_for('propertyValue', 'propmapkey')
def prop_val_mapkey_completer(ctxt, cass):
    completer_target = get_completer_target(ctxt)
    return prop_mapkey_completers.get(completer_target)(ctxt, cass)


@completer_for('propertyValue', 'propmapval')
def prop_val_mapval_completer(ctxt, cass):
    completer_target = get_completer_target(ctxt)
    return prop_mapval_completers.get(completer_target)(ctxt, cass)


@completer_for('propertyValue', 'ender')
def prop_val_mapender_completer(ctxt, cass):
    completer_target = get_completer_target(ctxt)
    return prop_mapender_completers.get(completer_target)(ctxt, cass)


def get_completer_target(ctxt):
    return ctxt.get_binding('wat').upper()
...
<cfamProperty> ::= <property>
                 ...
                 | "CLUSTERING" "ORDER" "BY" "(" <cfamOrdering>
                                                 ( "," <cfamOrdering> )* ")"
                 | "VERTEX" "LABEL" (<labelName>)?
                 | "EDGE" "LABEL" (<labelName>)? "FROM" <labelName> "(" <pkDef> ")"
                                                 "TO" <labelName> "(" <pkDef> ")"
                 ;
...
<compositeKeyCfSpec> ::= [newcolname]=<cident> <storageType>
                         ...
                         ( "," [newcolname]=<cident> <storageType> ( "static" )? )*
                         "," "PRIMARY" k="KEY" <compositePkDef>
                       ;

<compositePkDef> ::= p="(" ( partkey=<pkDef> | [pkey]=<cident> )
                          ( c="," [pkey]=<cident> )* ")"
                   ;
...
@completer_for('compositePkDef', 'pkey')
...
@completer_for('compositePkDef', 'c')
...
<alterInstructions> ::= "ADD" newcol=<cident> <storageType> ("static")?
                      ...
                      | "WITH" <cfamProperty> ( "AND" <cfamProperty> )*
                      | "WITHOUT" <graphLabelRemoval> ( "AND" <graphLabelRemoval> )*
                      | "RENAME" <renaming> ( "AND" <renaming> )*
                      ;
...
<graphLabelRemoval> ::= ( "EDGE" | "VERTEX" ) "LABEL" (<labelName>)? ;

<renaming> ::= existcol=<cident> "TO" newcol=<cident>
             | ("EDGE" | "VERTEX") "LABEL" (<labelName>)? "TO" <labelName>
             ;
'''
...
@completer_for('renaming', 'existcol')
def alter_table_rename_col_completer(ctxt, cass):
    layout = get_table_meta(ctxt, cass)
    cols = [str(md) for md in layout.columns]
...
explain_completion('renaming', 'newcol', '<new_column_name>')
...
syntax_rules += r'''
<backupRestoreStatement>        ::=   <createBackupConfigStatement>
                                    | <createBackupStoreStatement>
                                    | <dropBackupConfigStatement>
                                    | <dropBackupStoreStatement>
                                    | <alterBackupConfigStatement>
                                    | <alterBackupConfigInstructions>
                                    | <alterBackupStoreStatement>
                                    | <runBackupStatement>
                                    | <restoreBackupStatement>
                                    | <cancelBackupStatement>
                                    | <cancelRestoreStatement>
                                    | <listBackupConfigsStatement>
                                    | <listBackupStoresStatement>
                                    | <listBackupsStatement>
                                    | <verifyBackupStoreStatement>
                                    | <cleanBackupStoreStatement>
                                    ;

<backupConfigName>              ::=   <identifier>
                                    | <unreservedKeyword>
                                    ;

<backupStoreName>               ::=   <identifier>
                                    | <unreservedKeyword>
                                    ;

<properties>                    ::= "WITH" <property> ( "AND" <property> )*
                                    ;

<backupId>                      ::= <backupConfigName> /-[0-9]{14}\.[0-9]{3}/ ("-" <uuid>)?
                                    ;

<createBackupConfigStatement>   ::= "CREATE" "BACKUP" wat="CONFIGURATION" ( "IF" "NOT" "EXISTS" )? <backupConfigName>
                                    "OF" ksName=<keyspaceName> "TO" "STORE" <backupStoreName> ( "," <backupStoreName> )*
                                    <properties>
                                    ;

<createBackupStoreStatement>    ::= "CREATE" "BACKUP" wat="STORE" ( "IF" "NOT" "EXISTS" )? <backupStoreName>
                                    "USING" storeClass=<stringLiteral>
                                    <properties>
                                    ;

<dropBackupConfigStatement>     ::= "DROP" "BACKUP" "CONFIGURATION" ( "IF" "EXISTS" )? <backupConfigName>
                                    ;

<dropBackupStoreStatement>      ::= "DROP" "BACKUP" "STORE" ( "IF" "EXISTS" )? <backupStoreName>
                                    ;

<alterBackupConfigStatement>    ::= "ALTER" "BACKUP" wat="CONFIGURATION" <backupConfigName>
                                    <alterBackupConfigInstructions>
                                    ;

<alterBackupConfigInstructions> ::=   "ADD" "STORE" <backupStoreName>
                                    | "DROP" "STORE" <backupStoreName>
                                    | <properties>
                                    ;

<alterBackupStoreStatement>     ::= "ALTER" "BACKUP" wat="STORE" <backupStoreName>
                                    <properties>
                                    ;

<runBackupStatement>            ::= "RUN" "BACKUP" <backupConfigName>
                                    ;

<restoreBackupStatement>        ::= "RESTORE" <schemaElements>
                                    "FROM" "BACKUP" <backupId>
                                    "FROM" wat="STORE" <backupStoreName>
                                    ("USING" storeClass=<stringLiteral> <properties>)?
                                    ("INTO" <ksName>)?
                                    ;

<schemaElements>                ::= <schemaElement> ( "," <schemaElement> )*
                                    ;

<schemaElement>                 ::= <keyspaceName> ("." <cfOrKsName>)?
                                    ;

<cancelBackupStatement>         ::= "CANCEL" "BACKUP" <backupId>
                                    ;

<cancelRestoreStatement>        ::= "CANCEL" "RESTORE" <uuid>
                                    ;

<listBackupConfigsStatement>    ::= "LIST" "BACKUP" "CONFIGURATIONS"
                                    ;

<listBackupStoresStatement>     ::= "LIST" "BACKUP" "STORES"
                                    ;

<listBackupsStatement>          ::= "LIST" "BACKUPS" "FROM" "KEYSPACE" ksName=<keyspaceName>
                                    ;

<verifyBackupStoreStatement>    ::= "VERIFY" "BACKUP" "STORE" <backupStoreName>
                                    ;

<cleanBackupStoreStatement>     ::= "CLEAN" "BACKUPS"
                                     ("FOR" "CONFIGURATION" <backupConfigName>)?
                                     ("ON" "STORE" <backupStoreName>)?
                                    ;
'''


blob_store_implementations = ["'S3BlobStore'", "'FSBlobStore'", "'GoogleCloudBlobStore'"]


@completer_for('createBackupStoreStatement', 'storeClass')
def store_class_completer(ctxt, cass):
    return blob_store_implementations


@completer_for('restoreBackupStatement', 'storeClass')
def store_class_completer(ctxt, cass):
    return blob_store_implementations
