...
def ssl_settings(host, config_file, env=os.environ):
    ...
    * config_file ..: path to cqlsh config file (usually ~/.cqlshrc).
    ...
    def get_best_tls_protocol(ssl_ver_str):
        ...
        for protocol in ['PROTOCOL_TLS', 'PROTOCOL_TLSv1_3', 'PROTOCOL_TLSv1_2', 'PROTOCOL_TLSv1_1', 'PROTOCOL_TLSv1']:
    ...
    if ssl_version is None:
        sys.exit("%s is not a valid SSL protocol, please use one of "
                 "TLSv1, TLSv1_1, TLSv1_2, or TLSv1_3" % (ssl_version_str,))
...
def ssl_version_name(ssl_version):
    if ssl_version is None:
        return None
    name = ssl.get_protocol_name(ssl_version)
    if name == '<unknown>':
        return "Unknown SSL protocol '%s'" % (ssl_version,)
    return name[9:]  # remove the 'PROTOCOL_' prefix


def human_readable_ssl_options(ssl_options):
    if not ssl_options:
        return "none"
    validate = 'true' if ssl_options['cert_reqs'] == ssl.CERT_REQUIRED else 'false'
    version = ssl_version_name(ssl_options['ssl_version'])
    result = "validate: %s, version: %s" % (validate, version)
    if ssl_options['ca_certs']:
        result += ', certfile: %s' % (ssl_options['ca_certs'],)
    if ssl_options['keyfile']:
        result += ', userkey: %s' % (ssl_options['keyfile'],)
    if ssl_options['certfile']:
        result += ', usercert: %s' % (ssl_options['certfile'],)
    return result
